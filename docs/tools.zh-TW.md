...
| Codex | 固定版本上游 Linux-musl static binary | 明確選用的原生實驗；sandbox 另外驗證 |
...
直接執行有合理可行性；但這不等於上游正式支援 Android。僅缺少 Android build
target，也不能證明這些 Linux static binary 一定不能執行。
